...
def load_data(dataset, bfs_level=3, relabel=False):
    if dataset in ['spotify','credit','usa_big','FB15k-237']:
...
class RGCNLinkDataset(object):
    ...
    def load(self):
        ...
        neg_path = os.path.join(self.dir, 'neg.txt')
        ...
        # if 'usa' in self.name or 'fb_' in self.name:
        # neg_path = os.path.join(self.dir, 'neg.txt')
        # df_neg = pd.read_csv(neg_path, header=None, sep='\t')
        # M = df_neg.shape[0]
